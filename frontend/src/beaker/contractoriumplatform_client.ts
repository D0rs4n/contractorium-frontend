...
export class ContractoriumPlatform extends bkr.ApplicationClient {
	...
	async create_report(
		...
		const result = await this.execute(
			await this.compose.create_report({ to: args.to, description: args.description }, txnParams)
		);
	...
	compose = {
		...
		delete_report: async (
			...
			return this.addMethodCall(
				algosdk.getMethodByName(this.methods, 'delete_report'),
				{},
				txnParams,
				atc
			);
		...
		create_report: async (
			...
			return this.addMethodCall(
				...
				{ to: args.to, description: args.description },
				txnParams,
				atc
			);
		},
