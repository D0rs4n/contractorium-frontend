<script lang="ts">
	...
	async function delete_program_admin(address: string) {
		if (!contractoriumplatform_client || stored_wallet?.address === undefined) {
			return;
		}
		let res;
		loading_txn = true;
		try {
			res = await contractoriumplatform_client.delete_program_admin({program: address},{
				boxes: [
					{
						appIndex: parseInt(env.PUBLIC_APP_ID),
		...
		notifications.add('success', 'Successfully deleted the bounty program!', '');
		loading_txn = false;
		setTimeout(() => {
			window.location.href = '/';
		}, 2000);
...
>
	...
	<div class="">
		...
		{:else if stored_wallet && stored_wallet?.address == env.PUBLIC_MANAGER}
		<button
				class="outline-none border border-red-500 w-full bg-red-500 text-white px-3 py-2 rounded-lg flex flex-row mx-auto md:mx-0 md:mt-4 mt-8 transition-transform hover:scale-105"
				on:click={async () => {
					await delete_program_admin(data.address);
				}}
			>
				Delete bounty program (Admin override)<i class="material-icons ml-2">report </i>
			</button>
		{/if}
	</div>
</div>
