...
<form method="POST" action="?/editprogram">
	<label>
	  Bounty Program name
	  <input name="name" type="text" value={current_bounty.name}> <br>
	</label> <br>
	<label>
	  Bounty Program Description
	  <input name="description" type="text" value={current_bounty.description}> <br>
	</label> <br>
	<label>
		Bounty Program Image (URL as of now)
		<input name="image" type="text" value={current_bounty.image}> <br>
	  </label> <br>
	<button>Modify bounty!</button> <br>
  </form>
{/if}
{#if form?.success}
