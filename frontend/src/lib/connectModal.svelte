<script lang="ts">
	...
	export let connectStatus: string;
</script>
	...
	<div class="md:ml-8">
		{#if walletAddress !== undefined}
			<h1 class="text-xl text-white">
				{displayAlgoAddress(walletAddress)} |
				<button
					class="text-teal-600"
					on:click={() => {
						if (walletAddress !== undefined) {
				...
				>
					Hunter profile<i class="text-teal-600 material-icons ml-1 float-right">fingerprint</i
					></button
				>
			</h1>
		{/if}
...
{:else if connectStatus === 'connecting'}
	<div class="md:ml-10">
		<div>
			...
			<span class="text-white font-bold">Connecting..</span>
		</div>
	</div>
